...
namespace STMatch {
  ...
  struct GraphPreprocessor {
    ...
    void read_bin_file(std::string& filename) {
      ...
      if(KEY_NODE){
        g.keyrow=new graph_edge_t[n_vertices+1];
        read_subfile(filename + ".keyrow.bin",g.keyrow,n_vertices + 1);
        graph_edge_t keycolsize=g.keyrow[n_vertices];
        g.keycol=new graph_node_t[keycolsize];
        read_subfile(filename + ".keycol.bin",g.keycol,keycolsize);
      }
