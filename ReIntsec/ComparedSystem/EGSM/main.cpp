...
int main(int argc, char** argv) {
    ...
    std::cout<<"query Graph process done"<<std::endl;
    Graph data_graph(data_path, label_map);
    std::cout<<"data Graph process done"<<std::endl;
    ...
    std::cout << "程序执行时间1: " << duration.count() << " 毫秒" << std::endl;
    using namespace std;
    ofstream outputFile("../result_esgm.txt",ios::app);
    outputFile<<data_path<<' '<<query_path<<' '<<duration.count()<<' '<<res_size<<std::endl;
    outputFile.close();
    pool.Free();
